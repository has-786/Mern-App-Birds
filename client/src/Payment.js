...
if(!this.state.username){alert('Please Login First');}
 ...
 (body)=>{ var amt=0; body.map((prod)=>amt+=prod.price); this.setState({amount:amt});}).catch(err=>console.log(err));	
	}
}	
...
fun1=()=>{
  ...
  const currDate = date;
	var data={name:this.state.username,amount:this.state.amount,address:document.getElementById('address').value,timestamp:currDate};
...
fun2=()=>{
  ...
  alert(username)
    let options = {
     ...
     var data={paymentId:response.razorpay_payment_id,name:username,amount:0,address:document.getElementById('address').value,timestamp:currDate};
